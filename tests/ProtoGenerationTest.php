...
final class ProtoGenerationTest extends TestCase
{
    ...
    /**
     * The two generated trees must hold nothing but protoc's output.
     *
     * This is the one misfiling the layout cannot survive: a hand-written class
     * placed in `src/Proto/` or `metadata/` is deleted by the next run of
     * bin/generate-protos.sh, which removes both directories and rewrites them.
     * It is also excluded from Pint and PHPStan, so it stops being checked on the
     * way in. Nothing announces either.
     *
     * It has happened once already. `ProtocolVersion` lived in `src/Proto/`
     * because that is where the generator wrote it, and sat unlinted there until
     * someone went looking. The rule is in CONTRIBUTING; this is what enforces it.
     *
     * @return iterable<string, array{string}>
     */
    public static function generatedTreeProvider(): iterable
    {
        foreach (self::generatedFiles() as $path) {
    ...
    #[\PHPUnit\Framework\Attributes\DataProvider('generatedTreeProvider')]
    public function test_a_file_in_a_generated_tree_came_from_protoc(string $path): void
    {
        ...
        self::assertStringContainsString(
            ...
            $head,
            sprintf(
                '%s is in a generated tree but was not written by protoc. It will be deleted by the '
                . 'next bin/generate-protos.sh, and it is excluded from Pint and PHPStan in the meantime. '
                . 'Hand-written code belongs in src/, outside src/Proto/.',
    ...
    {
        // Both generated roots. Scanning only src/Proto would leave the 16
        // descriptor files unchecked, and the checks above would still pass --
        // metadata/ happens to call no GPBUtil helpers, so its absence was
        // invisible until a hand-written file was planted there and not caught.
        foreach (['/src/Proto', '/metadata'] as $relative) {
            $root = dirname(__DIR__) . $relative;
